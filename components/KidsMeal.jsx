...
function Fridge({ addToCart }) {
  ...
  const [results, setResults] = useState([]);   // 이번 추천 3개
  const [idx, setIdx]         = useState(0);    // 현재 보여줄 인덱스
  const [seen, setSeen]       = useState([]);   // 이미 추천한 메뉴 (중복 방지)
  ...
  async function generate(currentSeen = seen) {
    ...
    const avoidLine = currentSeen.length
      ? `\n이미 추천한 메뉴라 반드시 제외: ${currentSeen.join(", ")}`
      : "";

    const prompt =
      `너는 창의적인 어린이 영양사야. 냉장고 재료: ${items.join(", ")}\n` +
      `이 재료를 최대한 활용해서 아이가 좋아할, 서로 완전히 다른 장르의 메뉴 3가지를 추천해줘 ` +
      `(예: 볶음밥·파스타·국물요리처럼 다양하게). 간은 순하게, 탄수화물·단백질·채소 균형 맞게.` +
      avoidLine +
      `\n반드시 아래 JSON 스키마 배열로만 반환:` +
      `\n[{"dish":"메뉴명","time":"조리시간","description":"아이에게 왜 좋은지 한 줄","uses":["실제 쓰는 재료"],"extra":["추가 재료, 없으면 빈배열"],"steps":["1단계","2단계","3단계","4단계"],"tip":"팁"}]`;
  ...
  const next = () => {
    ...
    else generate();   // 3개 다 봤으면 새 추천 (already-seen 제외)
  };
  ...
  return (
    <div className="stack">
      <article className="card intro">
        ...
        <p className="intro-text">남은 재료를 적어주세요. 그 재료로 만들 메뉴를 골라 드릴게요.</p>
        <div className="input-wrap">
          <input className="ing-input" value={input} onChange={e => setInput(e.target.value)}
            onKeyDown={onKey} placeholder="재료 입력 후 Enter" />
          <button className="add-btn" onClick={() => add(input)} aria-label="추가"><Plus size={16} /></button>
        </div>
        {items.length > 0 && (
          <div className="ing-list">
            {items.map(v => (
              <span className="ing-chip" key={v}>{v}
                <button onClick={() => remove(v)} aria-label="삭제"><X size={12} /></button>
              </span>
            ))}
          </div>
        )}
        <div className="preset">
          ...
          <div className="preset-row">
            {PRESET.filter(p => !items.includes(p)).map(p => (
              <button className="preset-chip" key={p} onClick={() => add(p)}>+ {p}</button>
            ))}
          </div>
        </div>
        <button className="cook-btn" disabled={items.length === 0 || loading} onClick={() => generate()}>
          {loading
            ? <><Loader2 size={16} className="spin" /> 메뉴 고르는 중…</>
            : <>이 재료로 메뉴 추천받기 <ChevronRight size={16} /></>}
        </button>
      </article>
      ...
      {cur && (
        <article className="card result" key={cur.dish}>
          <div className="result-nav">
            <span className="mono small-label">{idx + 1} / {results.length} 추천</span>
            <button className="next-btn" onClick={next}>
              {idx < results.length - 1 ? "다음 메뉴 ›" : "다른 메뉴 추천 ›"}
            </button>
          </div>
          <div className="result-head">
            <div className="dish">{cur.dish}</div>
            {cur.time && <span className="mono time-tag">{cur.time}</span>}
          </div>
          {cur.description && <p className="note">{cur.description}</p>}
          {Array.isArray(cur.uses) && cur.uses.length > 0 && (
            <div className="rsec">
              <span className="mono small-label">있는 재료로</span>
              <div className="ing-list">{cur.uses.map(u => <span className="chip" key={u}>{u}</span>)}</div>
            </div>
          )}
          {Array.isArray(cur.extra) && cur.extra.length > 0 && (
            <div className="rsec">
              <span className="mono small-label dim">추가로 있으면 좋아요</span>
              <div className="ing-list">{cur.extra.map(u => <span className="chip ghost" key={u}>{u}</span>)}</div>
            </div>
          )}
          {Array.isArray(cur.steps) && (
            <div className="rsec">
              <span className="mono small-label">레시피</span>
              <ol className="steps">
                {cur.steps.map((s, i) => (
                  <li key={i}><span className="mono step-n">{String(i + 1).padStart(2, "0")}</span>{s}</li>
                ))}
              </ol>
            </div>
          )}
          {cur.tip && (
            <div className="tip"><span className="mono small-label">한 가지 팁</span><p>{cur.tip}</p></div>
          )}
          <button
            className={`cart-add wide ${added ? "ok" : ""}`}
            onClick={() => {
              addToCart([...(cur.uses || []), ...(cur.extra || [])]);
              setAdded(true);
